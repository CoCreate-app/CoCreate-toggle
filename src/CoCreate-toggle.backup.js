...
const CoCreateToggle = {
	...
	initElement: function(container) {
		...
		let elements = mainContainer.querySelectorAll('[data-toggle]');
		if (elements.length === 0 && mainContainer != document && mainContainer.hasAttributes('data-toggle')) {
			elements = [mainContainer];
		}
		
		elements.forEach((element) => self.__initEachElement(element));
	},
	
	__initEachElement: function(element) {
		let values = element.dataset['toggle'].split(',').map(x => x.trim());
